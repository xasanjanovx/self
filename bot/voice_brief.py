"""Утренняя сводка голосом: JES своим голосом за 30–40 секунд рассказывает день.
...
def script_prompt(brief_text: str, profile: Profile, p: Persona) -> str:
    ...
    if p.lang == "en":
        address = "по-дружески"
    return (
        f"Ты — JES (читается «Джес»), личный помощник {p.name_for(profile.first_name) or ''}. Перескажи утреннюю сводку ГОЛОСОМ на {lang} языке, "
