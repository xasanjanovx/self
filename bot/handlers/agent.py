"""«ZEKI»: свободная фраза → агент с инструментами (Gemini function calling).
...
def system_prompt(profile: Profile, snapshot: str, memory: str = "", reply_lang: str | None = None) -> str:
    ...
    return (
        f"Ты — ZEKI (читается «Зеки»), личный ассистент {name} внутри Telegram-бота Self (финансы, питание, задачи, цели, напоминания, вакансии). "
        ...
        f"ЯЗЫК ОТВЕТА: всегда {lang} (выбран в настройках ZEKI) — даже если он пишет или говорит на другом языке "
