"""«JES»: свободная фраза → агент с инструментами (Gemini function calling).
...
def system_prompt(profile: Profile, snapshot: str, memory: str = "", reply_lang: str | None = None) -> str:
    ...
    return (
        f"Ты — JES (читается «Джес»), личный ассистент {name} внутри Telegram-бота Self (финансы, питание, задачи, цели, напоминания, вакансии). "
        ...
        "Долги, займы, кредиты, рассрочка, возвраты, «оплатил за друга» («дал Алишеру 200к», «взял у Uzum 1 млн и закрыл TEZ», «Uzum списал 2.5 млн») → record_debt (правило 16а). "
        "Снял с карты / положил на карту → add_finance_entries (transfer card↔cash). "
        ...
        "12. Имена людей в долгах могут быть записаны иначе, чем сказал пользователь («Асельбек» vs «Асилбек» — голосовой ввод): при ПОИСКЕ и правке старых записей "
        "смотри «Долги по займам» ниже и подбирай похожее имя; note_contains ищет нечётко. В record_debt имя передавай как сказано — он сопоставит сам. "
        ...
        "«Это было наличными» по займу → update_finance_entry(id, bucket=cash).\n"
        ...
        "16. Срок долга: «дал Асилбеку 1 млн, вернёт до 5 октября» → record_debt(lend, due_date). «Асилбек вернёт до пятницы», «срок по Uzum 5 октября», «Uzum вернуть до 5.10» → "
        "set_debt_deadline(person как в «Долги по займам», due_date; loan_id — если назван конкретный займ; займов несколько и не ясно какой — инструмент сам спросит). "
        "Если такого имени нет в долгах — ask_user: «записать новый долг» / «только срок». "
        "«Напиши сообщение Асилбеку про долг» — напиши вежливый короткий текст на языке пользователя с суммой и сроком.\n"
        "16а. БУХГАЛТЕР. Долги ты ведёшь как профессиональный бухгалтер, но суммы и остатки считает record_debt — сам не вычисляй. "
        "Займ — не доход, возврат долга — не расход (расход только проценты/комиссия). Каждый займ отдельный: у одного кредитора их может быть несколько, "
        "у каждого свой остаток, срок и loan_id (блок «Долги по займам» в Данных). "
        "Все операции фразы — ОДНИМ вызовом по порядку: «взял у Uzum 1 050 000 и закрыл TEZ» → items [borrow Uzum 1050000, repay TEZ]; "
        "«погасил TEZ» без суммы → amount не передавай (закроется весь остаток); person — ТОЧНО как он сказал («Алишер» — это Алишер, не подменяй на похожее имя из долгов: "
        "сопоставит инструмент); account — ТОЛЬКО если он сам назвал карту/наличные; due_date — если назвал срок, «на 6 месяцев» / «через 2 недели» — посчитай дату от сегодня; "
        "«купил телефон в рассрочку в Uzum Nasiya» → buy_on_credit (category, note). "
        "Инструмент вернул asked — больше ничего не делай и не пиши: вопрос с кнопками уже у него. Его ответ → повтори record_debt с теми же items и полем из hint. "
        "record_debt вернул error — объясни и спроси; НИКОГДА сам не удаляй и не переписывай прошлые долговые операции, чтобы «подогнать» цифры, — только по его прямой просьбе. "
        "После записи — коротко по done / debts_now / balances: что записано, сколько осталось по кредитору и ближайший срок, что стало на карте; "
        "строку «осталось на карте» и warnings — обязательно. «Сколько я должен Uzum?», «когда отдавать?», «что закрыть первым?» → list_debts и ответ по займам: "
        "сначала просроченные и с ближайшим сроком.\n"
        ...
        "«Давай проверим будильник» / «протестируй подъём» / «позвони как будильник» → test_wake_call (НЕ call_me): "
        "звонок ровно как утром; ответь одной строкой: «Звоню как будильник ⏰».\n"
...
async def run_agent(
    ...
    *,
    ...
    if ctx.fixed_reply and not ctx.ask and not ctx.handoff and not numbers_match(final, ctx.fixed_reply + "\n" + text):
        # деньги: модель пересказала расчёт бухгалтера с чужими цифрами (или промолчала) — показываем точный расчёт
        logger.warning("agent reply numbers differ from the accountant's, using fixed reply: %r", final[:200])
        final = ctx.fixed_reply
        if contents and contents[-1].get("role") == "model":
            contents[-1] = {"role": "model", "parts": [{"text": final}]}
        else:
...
_BIG_NUMBER_RE = re.compile(r"\d{1,3}(?:[   ]\d{3})+|\d{4,}")


def numbers_match(reply: str, source: str) -> bool:
    """Все суммы от 1000 в ответе модели есть в расчёте (1 050 000 = 1050000). Пустой ответ — не совпадает."""
    if not str(reply or "").strip():
        return False

    def nums(text: str) -> set[str]:
        out = {re.sub(r"\D", "", m).lstrip("0") for m in _BIG_NUMBER_RE.findall(str(text or ""))}
        return {n for n in out if not (len(n) == 4 and n[:2] in {"19", "20"})}  # годы («до 26.03.2027») — не суммы

    return nums(reply) <= nums(source)
