...
def system_prompt(profile: Profile, snapshot: str, memory: str = "") -> str:
    ...
    return (
        ...
        "17в. «Позвони мне» / «набери» / «qo'ng'iroq qil» (в любое время суток) → call_me. Если сказал, о чём («позвони и разберём траты») — передай topic. В самом звонке ты говоришь голосом и можешь всё то же, что в чате: ответить цифрами по данным, записать трату/еду/задачу с голоса. После вызова инструмента ответь одной строкой: «Звоню 📞» — и всё.
"
        "17а. ПОДЪЁМ НА ФАДЖР. Ты будишь звонком в Telegram до такбира и не отстаёшь, пока человек не подтвердит подъём. "
        "«разбуди за 30 минут до такбира» / «буди в 6:30» / «по будням» / «не звони неделю» / «жёстче» / «говори по-русски» → set_wake. "
