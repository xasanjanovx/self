"""«Только мой голос»: отпечаток голоса владельца и проверка, что «JES» сказал именно он.

Модель CAM++ (3D-Speaker, sherpa-onnx) превращает запись в вектор голоса; похожесть — косинус.
Отпечаток — среднее по записи: 10 раз «JES» + ~40 с чтения текста (короткое «JES» само по себе
...
Сырая похожесть на коротком «JES» у чужих голосов бывает до ~0.58, поэтому решаем по нормированной:
...
JES работает как раньше. Отпечаток — DATA_DIR/voiceprint_<uid>.json.
"""
...
def calibrate(scores: list[float]) -> float:
    """Порог по его же коротким «JES»: чуть ниже худших 10% — но в разумных рамках."""
    import numpy as np
...
async def enroll(uid: int, wake_wavs: list[bytes], reading_wav: bytes | None) -> dict[str, Any]:
    """Записать отпечаток: короткие «JES» + длинное чтение. Возвращает порог и оценки."""
    import numpy as np
...
# Образец ЭТОГО разговора: его «JES», подтверждённое при пробуждении, и его же уверенные фразы. Тот же телефон, та же
...
def remember(uid: int, emb: Any) -> None:
    """Его голос в этом разговоре (подтверждённое «JES» или его уверенная фраза) — образец для следующих фраз."""
    if emb is None:
        return
...
async def warm() -> None:
    """При запуске: модель в память заранее (первое «JES» не ждёт загрузки)."""
    await extractor()
