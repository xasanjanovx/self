"""Экономный режим Nurai на телефоне: команды и короткие ответы — без Gemini Live.
...
CHEAP_RULES = (
    "\nЭКОНОМНЫЙ РЕЖИМ. Его реплика приходит записью голоса (в начале может прозвучать «Nurai» — это обращение, не просьба) "
    ...
    "в записи нет слов вовсе (тишина, шум) или прозвучало одно имя «Nurai».\n"
...
class PhoneCheap:
    def __init__(self, profile, persona, phone_ws, device: dict[str, Any], memory: str) -> None:  # noqa: ANN001
        ...
        # «только мой голос» посреди разговора выключено (25.09): на длинных фразах z-норма отсекала его самого
        # (сходство 0.85, z −0.2) — Джарвис молчал после «Да, шеф». Чужих отсекает проверка на «Джарвис» (wake_check).
