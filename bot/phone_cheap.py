"""Экономный режим ZEKI на телефоне: команды и короткие ответы — без Gemini Live.
...
OWNER_CHECK = True        # «только мой голос» и посреди разговора (26.09, строго — его выбор)
...
CHEAP_RULES = (
    "\nЭКОНОМНЫЙ РЕЖИМ. Его реплика приходит записью голоса (в начале может прозвучать «ZEKI» — это обращение, не просьба) "
    ...
    "в записи нет слов вовсе (тишина, шум) или прозвучало одно имя «ZEKI».\n"
...
class PhoneCheap:
    def __init__(self, profile, persona, phone_ws, device: dict[str, Any], memory: str) -> None:  # noqa: ANN001
        ...
        # «только мой голос» посреди разговора: 25.09 z-норма отсекала его самого (сходство 0.85, z −0.2); с 26.09
        # высокая сырая похожесть и сходство с его «ZEKI» этого разговора (voiceprint.is_other) решают в его пользу.
        self.owner_check = OWNER_CHECK and voiceprint.enrolled(self.uid)
        self.busy = False
        self.ended = False
        self.closed = False
