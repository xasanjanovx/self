"""Экономный режим JES на телефоне: команды и короткие ответы — без Gemini Live.
...
CHEAP_RULES = (
    "\nЭКОНОМНЫЙ РЕЖИМ. Его реплика приходит записью голоса (в начале может прозвучать «JES» — это обращение, не просьба) "
    ...
    "в записи нет слов вовсе (тишина, шум) или прозвучало одно имя «JES».\n"
...
class PhoneCheap:
    def __init__(self, profile, persona, phone_ws, device: dict[str, Any], memory: str) -> None:  # noqa: ANN001
        ...
        # высокая сырая похожесть и сходство с его «JES» этого разговора (voiceprint.is_other) решают в его пользу.
