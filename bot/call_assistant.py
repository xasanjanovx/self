"""Звонок по требованию: «позвони» → JES звонит и говорит с тобой голосом.
...
async def _agent_answer(profile: Profile, text: str, history: list[dict[str, Any]], snapshot: str) -> tuple[str, list[dict[str, Any]]]:
    """Ответ «JES» со всеми инструментами — как в чате, но короткий, для трубки."""
...
_FIX_RU = ("Сделай один раз: 1) открой аккаунт JES (кнопка ниже или чат от него) → «Добавить в контакты»; "
           "2) Telegram → Настройки → Конфиденциальность → Звонки → «Все» (или «Мои контакты»).")
_FIX_UZ = ("Bir marta qiling: 1) JES akkauntini oching (pastdagi tugma yoki undan kelgan chat) → «Kontaktga qo'shish»; "
           "2) Telegram → Sozlamalar → Maxfiylik → Qo'ng'iroqlar → «Hamma» (yoki «Kontaktlarim»).")
_DEVICE_RU = ("Если телефон не звонил, а пришёл только «пропущенный»: добавь JES в контакты; на iPhone выключи "
              "Настройки → Телефон → «Заглушение неизвестных»; на Android разреши Telegram уведомления о звонках и работу в фоне.")
_DEVICE_UZ = ("Telefon jiringlamay, faqat «o'tkazib yuborilgan» kelgan bo'lsa: JESni kontaktlarga qo'shing; iPhone'da "
...
HELPER_INTRO = {
    "ru": ("Ассалому алайкум! Я — JES, помощник из бота. С этого аккаунта я вам звоню (будильник и разговор). "
    ...
    "uz": ("Assalomu alaykum! Men — JES, botdagi yordamchi. Shu akkauntdan sizga qo'ng'iroq qilaman (budilnik va suhbat). "
    ...
    "en": ("Assalamu alaikum! I'm Jes, the assistant from the bot. I call you from this account (alarm and conversations). "
...
async def helper_intro(profile: Profile) -> bool:
    """Аккаунт JES один раз пишет человеку: появляется чат с кнопкой Telegram «Добавить в контакты»
...
async def _notify_failure(profile: Profile, error: str) -> None:
    ...
    reasons = {
        ...
        "peer_unknown": ("Аккаунт JES тебя ещё «не знает». " + _FIX_RU, "JES akkaunti sizni hali tanimaydi. " + _FIX_UZ),
        "privacy": ("Telegram не пропустил звонок: твои настройки «Кто может мне звонить» запрещают аккаунту JES. " + _FIX_RU,
                    "Telegram qo'ng'iroqni o'tkazmadi: «Kim menga qo'ng'iroq qila oladi» sozlamasi JES akkauntiga ruxsat bermayapti. " + _FIX_UZ),
        "no_answer": ("Звонила, но трубку не взяли. " + _DEVICE_RU, "Qo'ng'iroq qildim, lekin javob bo'lmadi. " + _DEVICE_UZ),
        "daily_limit": ("Дневной лимит на живой голос исчерпан — позвоню завтра. Пишите здесь или зовите «JES» на телефоне (экономный режим).",
                        "Jonli ovoz uchun kunlik limit tugadi — ertaga qo'ng'iroq qilaman. Shu yerda yozing yoki telefonda «JES» deng."),
    }
    ...
    if fixable and caller.helper_username:
        ru += f" Аккаунт JES: @{caller.helper_username}"
        uz += f" JES akkaunti: @{caller.helper_username}"
    if fixable:
...
async def show_home(profile: Profile, notice: str | None = None, *, undo: bool = False, helper_button: bool = False) -> None:
    ...
    которые потом висят в чате. Заодно убирает экран JES, с которого звонили."""
    try:
...
async def _remember_call(profile: Profile, transcript: list[str]) -> None:
    ...
    чтобы в чате JES знал, о чём говорили («как я сказал по телефону…»)."""
    ...
    try:
        ...
        history.append({"role": "user", "parts": [{"text": f"(разговор по телефону с JES только что:\n{text})"}]})
...
def call_in_background(profile: Profile, *, topic: str = "", lang: str | None = None) -> asyncio.Task | None:
    ...
    другой язык, иначе берётся из настроек JES.
    """
    uid = profile.telegram_id
    if uid in _running:
        return None
...
async def on_incoming_call(user_id: int) -> bool:
    """Он сам позвонил аккаунту JES в Telegram — берём трубку и говорим (Gemini Live).
...
async def start_listening() -> None:
    """При запуске бота: аккаунт JES в сети и берёт трубку, когда владелец звонит сам."""
