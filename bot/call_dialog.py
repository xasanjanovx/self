"""Живой разговор в Telegram-звонке: ZEKI говорит, слушает ответ и отвечает.

Как устроено:
  play()   — то, что ZEKI говорит (синтез Gemini TTS → PCM → файл);
  ...
  далее: расшифровка (Gemini) → короткий ответ ZEKI → снова play().
...
def system_prompt(state: DialogState) -> str:
    ...
    return (
        f"Ты — ZEKI (читается «Зеки»), личный помощник {state.name or 'пользователя'}. Сейчас раннее утро, ты ЗВОНИШЬ ему по телефону, "
