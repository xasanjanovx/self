"""Живой разговор в Telegram-звонке: Джарвис говорит, слушает ответ и отвечает.

Как устроено:
  play()   — то, что Джарвис говорит (синтез Gemini TTS → PCM → файл);
  ...
  далее: расшифровка (Gemini) → короткий ответ Джарвиса → снова play().

Разговор короткий и с одной целью: убедиться, что человек реально встал (ответил
осмысленно, выпил воду, сделал приседания), и не дать снова лечь. Он заканчивается,
...
def nudge(state: DialogState) -> str:
    ...
    uz = state.lang != "ru"
    steps_uz = ["Eshitmadim. Gapiring, iltimos.", "Turing, yotmang. Bir stakan suv iching.", "Oyoqqa turing va menga gapiring."]
    steps_ru = ["Не слышу. Скажи что-нибудь.", "Вставай, не ложись. Выпей стакан воды.", "Поднимайся и ответь мне."]
...
def system_prompt(state: DialogState) -> str:
    ...
    return (
        f"Ты — Джарвис, личный помощник {state.name or 'пользователя'}. Сейчас раннее утро, ты ЗВОНИШЬ ему по телефону, "
        ...
        f"Такбир: {state.takbir or 'скоро'}. Задание на утро: {state.task_text or 'просто встать'}.\n"
        "Твоя цель: убедиться, что он реально встал — попроси сказать что-то осмысленное, выпить воду, сделать задание. "
