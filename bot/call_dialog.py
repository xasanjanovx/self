"""Живой разговор в Telegram-звонке: Nurai говорит, слушает ответ и отвечает.

Как устроено:
  play()   — то, что Nurai говорит (синтез Gemini TTS → PCM → файл);
  ...
  далее: расшифровка (Gemini) → короткий ответ Nurai → снова play().
...
def system_prompt(state: DialogState) -> str:
    ...
    return (
        f"Ты — Nurai, личный помощник {state.name or 'пользователя'}. Сейчас раннее утро, ты ЗВОНИШЬ ему по телефону, "
