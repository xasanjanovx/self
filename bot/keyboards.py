...
TEXTS: dict[Lang, dict[str, str]] = {
    "ru": {
        ...
        "proactive_on": "💡 Подсказки ZEKI: вкл",
        "proactive_off": "💡 Подсказки ZEKI: выкл",
    ...
    "uz": {
        ...
        "proactive_on": "💡 ZEKI maslahatlari: yoniq",
        "proactive_off": "💡 ZEKI maslahatlari: o'chiq",
...
def main_menu_keyboard(lang: str = "ru", *, undo: bool = False) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=undo_rows + [
            ...
            [
                _btn("ZEKI" if lang == "uz" else "ZEKI", "menu:jarvis", style=P, icon=_pe.ID_JARVIS),
...
def settings_keyboard(lang: str = "ru", *, owner: bool = False) -> InlineKeyboardMarkup:
    """Главный экран настроек — только разделы бота. Всё про ZEKI (голос, будильник, звонки) —
    отдельно, в кнопке «ZEKI» главного меню. «Пользователи» — только владельцу."""
    uz = lang == "uz"
    rows = [
...
def invite_keyboard(lang: str, link: str) -> InlineKeyboardMarkup:
    ...
    share = "https://t.me/share/url?url=" + quote(link, safe="") + "&text=" + quote(
        "Shaxsiy yordamchi ZEKI — kirish uchun bosing" if uz else "Личный помощник ZEKI — нажми, чтобы войти")
...
def notify_keyboard(lang: str, *, morning: bool, morning_wake: bool, evening: bool, proactive: bool, voice: bool,
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [toggle(proactive, "💡 " + ("ZEKI maslahatlari" if uz else "Подсказки ZEKI"), "settings:toggle:proactive")],
...
def jarvis_hub_keyboard(lang: str, *, alert_calls: bool = False, morning_voice: bool = False,
    ...
    """Кнопка «ZEKI» в главном меню: всё про ZEKI в одном месте."""
    uz = lang == "uz"
...
def wake_settings_keyboard(lang: str, *, enabled: bool, call_enabled: bool, talk: bool, voice_lang: str,
    ...
    ZEKI будит мотивирующими словами и сам по голосу убеждается, что встал."""
    uz = lang == "uz"
    weekdays = {1, 2, 3, 4, 5}
...
def jarvis_settings_keyboard(lang: str, *, voice: str, call_lang: str, address: str, tone: str, verbosity: str,
    ...
    """«Голос и характер» ZEKI: режим телефона, модель живого голоса, голос, язык, ты/вы, как величать, тон, длина ответов."""
