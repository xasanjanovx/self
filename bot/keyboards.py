...
TEXTS: dict[Lang, dict[str, str]] = {
    "ru": {
        ...
        "proactive_on": "💡 Подсказки JES: вкл",
        "proactive_off": "💡 Подсказки JES: выкл",
    ...
    "uz": {
        ...
        "proactive_on": "💡 JES maslahatlari: yoniq",
        "proactive_off": "💡 JES maslahatlari: o'chiq",
...
def settings_keyboard(lang: str = "ru", *, owner: bool = False) -> InlineKeyboardMarkup:
    """Главный экран настроек — только разделы бота. Всё про JES (голос, будильник, звонки) —
    отдельно, в кнопке «JES» главного меню. «Пользователи» — только владельцу."""
    uz = lang == "uz"
    rows = [
...
def invite_keyboard(lang: str, link: str) -> InlineKeyboardMarkup:
    ...
    share = "https://t.me/share/url?url=" + quote(link, safe="") + "&text=" + quote(
        "Shaxsiy yordamchi JES — kirish uchun bosing" if uz else "Личный помощник JES — нажми, чтобы войти")
...
def notify_keyboard(lang: str, *, morning: bool, morning_wake: bool, evening: bool, proactive: bool, voice: bool,
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [toggle(proactive, "💡 " + ("JES maslahatlari" if uz else "Подсказки JES"), "settings:toggle:proactive")],
...
def jarvis_hub_keyboard(lang: str, *, alert_calls: bool = False, morning_voice: bool = False,
    ...
    """Кнопка «JES» в главном меню: всё про JES в одном месте."""
    uz = lang == "uz"
