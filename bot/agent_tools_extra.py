"""Инструменты «ZEKI», которые делают его ассистентом, а не только оператором БД:
...
@tool(
    "ai_status",
    "Про самого ZEKI: остаток предоплаты Gemini (AI Studio), на сколько дней хватит, расход сегодня/за месяц/в среднем, "
