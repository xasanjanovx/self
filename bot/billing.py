...
Кончились деньги — Gemini отвечает 402: сразу пишем в чат, а ZEKI говорит об этом в разговоре.
...
def _maybe_limit_alert(st: dict[str, Any], day: dict[str, Any]) -> None:
    ...
    _notify(f"🟡 <b>Лимит Gemini на сегодня — ${DAILY_LIMIT_USD:g} — достигнут.</b>\nДо полуночи живой голос (Gemini Live) выключен: "
            "ZEKI на телефоне выполняет команды и отвечает в экономном режиме, камера, экран и звонки «позвони мне» — завтра. "
...
def exhausted(detail: str = "") -> None:
    ...
    _notify("🔴 <b>Баланс Gemini закончился.</b>\nБот и ZEKI не могут думать и говорить, пока не пополните предоплату "
...
def voice_note() -> str:
    """Строка в системный промпт разговора: мало денег — пусть ZEKI скажет об этом сам (раз в 6 часов)."""
    s = status()
