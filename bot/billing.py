...
Кончились деньги — Gemini отвечает 402: сразу пишем в чат, а JES говорит об этом в разговоре.
...
def exhausted(detail: str = "") -> None:
    ...
    _notify("🔴 <b>Баланс Gemini закончился.</b>\nБот и JES не могут думать и говорить, пока не пополните предоплату "
...
def voice_note() -> str:
    """Строка в системный промпт разговора: мало денег — пусть JES скажет об этом сам (раз в 6 часов)."""
    s = status()
