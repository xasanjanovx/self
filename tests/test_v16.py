...
@pytest.mark.parametrize("text, after", [
    ("джес", ""),
    ("эй джес", ""),
    ("джесс позвони маме", "позвони маме"),
    ("эйджес", ""),
    ("джейс какая погода завтра", "какая погода завтра"),
    ("жес открой ютуб", "открой ютуб"),
    ("jess open", "open"),
    # 26.09: его «Джес» распознаватель писал как «джест», «джаз» — это тоже имя (чужих отсекает проверка голоса)
    ("джест позвони маме", "позвони маме"),
    ("джаз позвони", "позвони"),
])
...
@pytest.mark.parametrize("text", [
    "жесть какая", "есть хочу", "здесь", "джек пришёл", "джинсы", "чес", "вес", "час",
    "бугун хаво джуда яхщи", "",
    # прежние имена больше не будят (он просил: только JES)
    "джарвис", "эй джарвис позвони маме", "нурай", "ну рай", "nurai", "зеки", "zeki",
])
